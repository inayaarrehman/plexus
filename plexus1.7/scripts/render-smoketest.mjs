...
check(`Systems renders the tactile node grid (all ${SYSTEMS.length} named systems)`, () => {
  ...
  // Each node carries its own abstract topology SVG and an accessible label.
  if (!html.includes('system-topology')) throw new Error('expected per-system topology SVGs in the grid')
